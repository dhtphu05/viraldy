...
def execute_structured_operation(
    ...
    *,
    ...
    runtime_examples = (
        render_runtime_few_shot_content(selected_examples)
    ...
    request_text = (
        stable_input
        if runtime_examples is None
        else f"{stable_input}\0{runtime_examples}"
    )
    ...
    request = StructuredGenerationRequest(
        ...
        input_hash=hashlib.sha256(request_text.encode("utf-8")).hexdigest(),
...
def provider_error_to_app_error(error: AiProviderError) -> AppError:
    info = error.info
    status_code = {
